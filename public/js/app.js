...
if (document.getElementById("form-update-reading")) {
  ...
  if (document.getElementById("infOldImages")) {
    ...
    sendOldImages = _multi_image_upload__WEBPACK_IMPORTED_MODULE_0__["oldImages"]; //se obtiene al formulario

    var fromUpdate = document.getElementById("form-update-reading"); //Se obtiene hacia donde esta enrutado el formulario

    var url = fromUpdate.getAttribute('action'); //Se obtienen los elementos inputs o textArea del formulario
    ...
    fromUpdate.addEventListener('submit', function (e) {
      ...
      axios.post(url, formData, confing).then(function (response) {
        ...
        descriptionElem.nextElementSibling.classList.add('hidden');
        uploadElem.nextElementSibling.classList.add('hidden');
        console.log('Registro actualizado'); // funciona como una redirección HTTP
